...
def simulate_git_project_processing(project_name, git_url):
    ...
    os.makedirs(os.path.join(base, "uploaded-files", "cpp"), exist_ok=True)
    os.makedirs(os.path.join(base, "unpacked", "cpp"), exist_ok=True)

    # 2. Write git_config.json (no branch)
    git_cfg = {"url": git_url, "branch": None}
    with open(os.path.join(base, "uploaded-files", "cpp", "git_config.json"), "w") as f:
        json.dump(git_cfg, f)
